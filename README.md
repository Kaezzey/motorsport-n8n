...
This repository contains the completed Milestones 1–4 plus a thin end-to-end prototype for **Motorsport Workbench Part 4**. It is evidence of the architecture and responsible-automation approach, not a claim that the later cross-channel diagnostics are production-ready.
...
- Validates source lap boundaries against a versioned Queensland Raceway geofence and duration profile.
- Scores `out_lap`, `in_lap`, `push_lap`, or `installation_lap` from telemetry features, compares against independent run-sequence weak labels, and routes uncertain or disagreeing classifications to review.
- Reports context-aware `vehicle_stop`, `wheel_lock_candidate`, `wheelspin_candidate`, and `course_excursion` events.
...
flowchart LR
    ...
    A -->|accept or review| B["Track-aware lap segmentation"]
    B --> C["n8n webhook"]
    C --> D["Bounded diagnostic controller"]
    D --> E["Deterministic policy + sensor/track profiles"]
    E --> F{"Decision"}
...
Open [http://localhost:5678](http://localhost:5678), open **Telemetry Quality Agent - Milestone 4**, and activate it. Submit the clean example:
...
Evaluate every copied lap locally through the Milestone 4 policy without starting n8n or writing audit events:

```bash
npm run evaluate -- data/incoming-manifests --output .local/milestone-4-evaluation.json
```

All 123 source lap boundaries validate against the track profile. The telemetry classifier agrees with the independent sequence labels on 122 laps; the sole disagreement has low confidence and is automatically reviewed. Six classifications and two wheelspin candidates request context review. Existing Milestone 3 sensor findings mean the complete copied event still produces 123 `review` decisions and zero downstream authorizations.
...
| `config/lap-context-profile.json`        | Track boundary, lap-classification, confidence, and event-taxonomy policy            |
...
| `src/lap-context.js`                     | Track-aware boundary evidence, classification, weak-label comparison, and events     |
...
- Lap classification is a calibrated deterministic scorer, not a trained classifier; its sequence labels are weak references rather than engineer ground truth.
